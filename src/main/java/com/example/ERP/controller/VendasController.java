...
public class VendasController {
    ...
    public void initialize() {
        ...
        // bind do botão: desabilita se NÃO houver resultados
        btnAdicionar.disableProperty().bind(Bindings.createBooleanBinding(() -> resultados.isEmpty(), resultados));

        // ao selecionar produto, ajustar máximo do spinner e habilitar botão (binding cuida da habilitação)
        tabelaResultados.getSelectionModel().selectedItemProperty().addListener((obs, oldS, newS) -> {
            atualizarSpinnerParaProduto(newS);
        });

        // permitir adicionar ao carrinho com duplo-clique na linha
    ...
    private void onBuscarProduto() {
        ...
        try {
            ...
            System.out.println("DEBUG: onBuscarProduto - found " + (list==null?0:list.size()) + " items for query='" + q + "'");
            resultados.setAll(list);
            buildSuggestions(list);
            // auto-select first result so spinner and selection are initialized
            if (list != null && !list.isEmpty()) {
                Product first = list.get(0);
                tabelaResultados.getSelectionModel().select(first);
                tabelaResultados.scrollTo(first);
                atualizarSpinnerParaProduto(first);
            } else {
                // clear spinner/state when no results
                atualizarSpinnerParaProduto(null);
            }
    ...
    private void buildSuggestions(List<Product> list) {
        ...
        // Only show the suggestionMenu if the text field is attached to a Scene (prevents NPE in headless/tests)
        try {
            if (txtBuscaProduto.getScene() != null && !suggestionMenu.isShowing()) {
                suggestionMenu.show(txtBuscaProduto, Side.BOTTOM, 0, 0);
            }
        } catch (Exception ex) {
            // ignore: in non-visual/test environments getScene() may be null or show() may fail
        }
    }

    // sincroniza ObservableList usado pela TableView com o serviço de carrinho
    private void refreshCarrinhoFromService() {
        Platform.runLater(() -> carrinho.setAll(servicoCarrinho.getItens()));
    }
    ...
    private void onAdicionarCarrinho() {
        Product sel = tabelaResultados.getSelectionModel().getSelectedItem();
        // If nothing selected but there are results, auto-select the first one
        if (sel == null && !resultados.isEmpty()) {
            sel = resultados.get(0);
            tabelaResultados.getSelectionModel().select(sel);
            tabelaResultados.scrollTo(sel);
        }
        System.out.println("DEBUG: onAdicionarCarrinho invoked");
        System.out.println("DEBUG: selected product = " + (sel==null?"null":sel.getId() + " - " + sel.getName()));
        ...
        System.out.println("DEBUG: requested quantity = " + qtd);
        // validate available
        int jaNoCarrinho = quantidadeNoCarrinho(sel.getId());
        int disponivel = sel.getStockQty() - jaNoCarrinho;
        System.out.println("DEBUG: stock=" + sel.getStockQty() + ", alreadyInCart=" + jaNoCarrinho + ", available=" + disponivel);
        if (qtd > disponivel) {
            mostrarAviso(String.format("Quantidade solicitada (%d) excede o disponível (%d)", qtd, disponivel));
            return;
        }
        try {
            servicoCarrinho.adicionar(sel.getId(), qtd);
            System.out.println("DEBUG: servicoCarrinho.adicionar succeeded");
            refreshCarrinhoFromService();
            atualizarTotal();
            atualizarSpinnerParaProduto(sel);
            quantDesejada.remove(sel.getId()); tabelaResultados.refresh();
            mostrarInfo("Produto '" + sel.getName() + "' adicionado ao carrinho (x" + qtd + ")");
        } catch (Exception ex) {
            System.out.println("DEBUG: exception adding to cart: " + ex);
            ex.printStackTrace(System.out);
            mostrarErro("Não foi possível adicionar ao carrinho: " + ex.getMessage(), ex instanceof Exception ? (Exception)ex : new Exception(ex));
        }
    }
